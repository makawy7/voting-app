...
<div x-data="{ open: false }" x-cloak {{ '@' . $openModalEvent }}.window="open = true" @keydown.escape.window="open = false"
    {{ '@' . $actionCompletedEvent }}="open = false" class="relative z-10" aria-labelledby="modal-title" role="dialog"
    aria-modal="true">
    ...
    <div x-transition.origin.bottom.duration.500ms x-show="open" class="fixed inset-0 z-10 overflow-y-autp">
        ...
        <div class="flex min-h-full items-center justify-center">
            <div @click.away="open = false"
                ...
                <form wire:submit="{{ $actionMethodName }}" class="space-y-4 mt-6 mb-2">
                    <div class="flex space-x-3 px-6">
                        ...
                        <div class="pb-2 ">
                            <h3 class="text-xl font-semibold mb-1">{{ $modalTitle }}</h3>
                            <p>{{ $modalContent }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex space-x-3 bg-gray-100 justify-end py-2 px-6">
                        ...
                        <button type="submit"
                            class="py-2 px-4 {{ $submitButtonStyles }} text-white rounded-lg font-semibold border border-gray-700 disabled:opacity-50">{{ $submitButtonText }}</button>
                    </div>
